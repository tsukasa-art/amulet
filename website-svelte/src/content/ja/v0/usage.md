---
title: "CLI 使い方リファレンス"
description: "Amuletの全コマンド、フラグ、および使用パターンの詳細リファレンス。"
order: 3
---
...
> bash では `HISTCONTROL` は `ignorespace` または `ignoreboth` に設定した上でコマンドを先頭スペース付きで入力すると記録を抑制できますが、これはシェルや設定に依存し全環境で有効ではありません。zsh での相当オプションは `setopt HIST_IGNORE_SPACE` です。確実な方法を求めるなら、`echo` の使用自体を避け、シークレットストアやファイル経由での入力を選んでください。自分の環境で実際に記録されるか確認したい場合は、`echo test` を実行して `history` で確認してみてください。
