...
export async function withSecret<T>(
  ...
  callback: (secret: Buffer) => Promise<T> | T,
  options: AmuletOptions = {},
): Promise<T> {
  const secret = await spawnUnseal(
    options.binaryPath ?? 'amulet',
    key,
    vaultPath,
    passphrase,
  );
...
function spawnUnseal(
  ...
  return new Promise((resolve, reject) => {
    const child = spawn(binary, ['unseal', key, '--file', vaultPath], {
    ...
    child.on('close', (code) => {
      if (overLimit) return; // already rejected
      if (code !== 0) {
        zeroAndClear(chunks);
...
function zeroAndClear(chunks: Buffer[]): void {
